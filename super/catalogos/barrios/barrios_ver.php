...
// Traer barrios con su municipio
$stmt = $pdo->query("
    SELECT b.id, b.nombre, m.nombre AS municipio
    FROM barrios b
...
<div class="container mt-4">
    <div class="card-body card p-4 mb-4 rounded shadow text-center hero-card">
        <h2 class="fw-bold text-white">Catálogo de Barrios</h2>
        <p class="mb-0 text-light">Administra los barrios registrados en el sistema.</p>
    </div>

    <div class="mb-4 text-end">
        <a href="barrios_form.php" class="btn custom-btn px-4 py-2">+ Agregar Barrio</a>
    </div>
    ...
    <div class="card glass-card shadow-sm border-0 p-4">
        <div class="table-responsive">
            <table class="table-fixed w-full">
                ...
                <tbody>
                    <?php foreach ($barrios as $row): ?>
                        <tr>
                            <td class="px-4 py-2 align-middle"><?= $row['id'] ?></td>
                            <td class="px-4 py-2 align-middle"><?= htmlspecialchars($row['nombre']) ?></td>
                            <td class="px-4 py-2 align-middle"><?= htmlspecialchars($row['municipio']) ?></td>
                            <td class="px-4 py-2 align-middle">
                                <a href="barrios_form.php?id=<?= $row['id'] ?>" class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">Editar</a>
                                <a href="barrios_delete.php?id=<?= $row['id'] ?>" onclick="return confirm('¿Seguro que deseas eliminar?')" class="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700">Eliminar</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
