...
$stmt = $pdo->query("SELECT * FROM tipos_incidencias ORDER BY id");
$tipos = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>

<div class="container mt-4">
    <div class="card-body card p-4 mb-4 rounded shadow text-center hero-card">
        <h2 class="fw-bold text-white">Catálogo de Tipos de Incidencias</h2>
        <p class="mb-0 text-light">Administra los tipos de incidencias registradas en el sistema.</p>
    </div>

    <div class="mb-4 text-end">
        <a href="tipos_incidencias_form.php" class="btn custom-btn px-4 py-2">+ Agregar Tipo</a>
    </div>
                    ...
                    <?php foreach ($tipos as $row): ?>
                        <tr>
                            ...
                            <td class="px-4 py-2 align-middle">
                                <a href="tipos_incidencias_form.php?id=<?= $row['id'] ?>" class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">Editar</a>
                                <a href="tipos_incidencias_delete.php?id=<?= $row['id'] ?>" onclick="return confirm('¿Seguro que deseas eliminar?')" class="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700">Eliminar</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
