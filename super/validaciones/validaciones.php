...
$incidencias = $pdo->query("SELECT i.id, i.titulo, i.descripcion, i.fecha_ocurrencia, i.lat, i.lng, i.muertos, i.heridos, i.perdida, i.link_social, i.foto, u.nombre AS reportero
                            FROM incidencias i
                            JOIN usuarios u ON i.reportero_id = u.id
                            WHERE i.validada = 0
                            ORDER BY i.fecha_creacion DESC")->fetchAll(PDO::FETCH_ASSOC);
?>
...
<head>
    <meta charset="UTF-8">
    <title>Validación de Incidencias</title>
...
<div class="container mt-4">
    <div class="card-body card p-4 mb-4 rounded shadow text-center hero-card">
        ...
        <p class="mb-0 text-light">Aprobar o rechazar los reportes enviados por los usuarios.</p>
    </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if($inc['link_social']): ?>
                                    <a href="<?= htmlspecialchars($inc['link_social']) ?>" target="_blank">Link</a><br>
                                <?php endif; ?>
                                <?php if($inc['foto']): ?>
                                    <a href="<?= htmlspecialchars($inc['foto']) ?>" target="_blank">Foto</a>
                                <?php endif; ?>
                            </td>
                            <td>
